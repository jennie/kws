...
function groupLabel(list: { performances?: unknown[] }[]) {
  const allTouring = list.length > 0 && list.every((c) => (c.performances?.length ?? 0) > 0)
...
const grouped = computed(() => {
  ...
  const ordered: { series: string; id: string; label: string; concerts: typeof upcoming.value }[] = []
  const take = (series: string, list: typeof upcoming.value) => {
    ordered.push({ series, id: seriesId(series), label: groupLabel(list), concerts: list })
  }
...
onMounted(() => {
  const sections = Array.from(document.querySelectorAll<HTMLElement>('[data-jump-target]'))
...
<template>
  <div class="bg-paper-50">
    <div v-if="upcoming.length" class="mx-auto max-w-shell px-6 lg:px-10">
      <div class="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2 pt-10 pb-6 lg:pt-14">
        <h1 class="font-display text-3xl font-bold tracking-tight text-paper-900 lg:text-[3.25rem] lg:leading-none">
          Concerts &amp; events
        </h1>
        <p class="text-base text-paper-600"><span class="font-semibold text-paper-900">{{ performanceCount }} performances</span> across the 2026/27 season</p>
      </div>
      ...
      >
        <div class="aspect-[16/9] border-b border-paper-900 lg:aspect-auto lg:border-b-0 lg:border-r">
          <img
            v-if="next.image"
            :src="next.image"
            :alt="next.title"
        ...
        <div class="flex flex-col justify-center p-8 lg:p-10">
          <div class="mb-4 flex items-baseline justify-between gap-4">
            <span v-if="next.series" class="text-base font-medium text-paper-600">{{ next.series }}</span>
            <span class="kws-eyebrow kws-eyebrow--ink">Next concert</span>
          </div>
          <h2 id="lead-title" class="mb-4 font-display text-3xl font-bold leading-[1.02] tracking-tight text-paper-900 lg:text-[3rem]">
            {{ next.title }}
          </h2>
          <p v-if="next.description" class="mb-6 max-w-[46ch] text-lg text-paper-600">
            {{ next.description }}
          </p>
          <dl class="mb-7">
            <dt class="kws-eyebrow">Date</dt>
            <dd class="mt-1 text-base text-paper-800">{{ longDate(next.date) }} · {{ timeOf(next.date) }}</dd>
            <dt class="kws-eyebrow mt-4">Venue</dt>
            <dd class="mt-1 text-base text-paper-800">
              {{ leadVenue.name }}<br v-if="leadVenue.city"><span v-if="leadVenue.city" class="text-paper-600">{{ leadVenue.city }}</span>
            </dd>
          </dl>
          <div>
            <TicketButton :url="next.ticketUrl" :provider="next.ticketProvider" size="lg" />
          </div>
        </div>
      </section>
        ...
        <ul class="flex flex-wrap gap-x-6 gap-y-2">
          <li v-for="group in grouped" :key="group.id">
            <a
              ...
              :class="activeSeriesId === group.id ? 'font-semibold text-paper-900' : 'font-medium text-paper-700'"
            >
              {{ group.series }}
            </a>
          </li>
      ...
      <div class="space-y-12 pb-16 lg:space-y-16">
        <section v-for="group in grouped" :id="group.id" :key="group.series" data-jump-target class="scroll-mt-20">
          <div class="mb-6 flex items-baseline justify-between gap-4 border-b-2 border-paper-900 pb-3">
            <h2 class="font-display text-2xl font-semibold tracking-tight text-paper-900 lg:text-3xl">
              {{ group.series }}
            </h2>
    ...
    <div v-else class="mx-auto max-w-shell px-6 lg:px-10">
      <section aria-label="No upcoming concerts" class="py-20 text-center lg:py-28">
        <h1 class="mx-auto max-w-2xl font-display text-3xl font-bold leading-[1.15] tracking-tight text-paper-900 lg:text-4xl">
